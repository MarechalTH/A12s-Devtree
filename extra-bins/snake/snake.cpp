...
constexpr int MAX_SNAKE = 1000;
constexpr int MAX_FRUITS = 10;
constexpr int MAX_WALLS = 75;
constexpr int MAX_BUGS = 5;
constexpr int MAX_POWERUPS = 3;
constexpr int LOGIC_DELAY_US = 10000;
constexpr int FRAME_EVERY_N_TICKS = 10;
constexpr int HEAD_INDEX = 0;
constexpr int SAFE_ZONE_RADIUS = 5;
...
enum PowerUpType { INVULNERABILITY, SPEED_BOOST, SLOW_BUGS, AREA_EXPLOSION };
...
enum SymbolSet { SYMBOL_SET_UNICODE, SYMBOL_SET_ASCII };
...
static void move_cursor(int x, int y) {
    std::cout << "\x1b[" << (y + 1 + Y_OFFSET) << ';' << (x + 1) << 'H' << std::flush;
}
...
static void set_snake_color() {
    std::cout << "\x1b[38;5;156m";
}

static void set_fruit_color() {
    std::cout << "\x1b[38;5;217m";
}

static void set_wall_color() {
    std::cout << "\x1b[38;5;180m";
}

static void set_border_color() {
    std::cout << "\x1b[38;5;223m";
}

static void set_bug_color(RatMode mode) {
    switch (mode) {
        case RAT_ERRATIC:     std::cout << "\x1b[38;5;75m"; break;
        case RAT_STALKER:     std::cout << "\x1b[38;5;166m"; break;
        case RAT_AMBUSHER:    std::cout << "\x1b[38;5;124m"; break;
        case RAT_PANIC:       std::cout << "\x1b[38;5;178m"; break;
        case RAT_PACK_HUNTER: std::cout << "\x1b[38;5;36m"; break;
        case RAT_TRAP_MASTER: std::cout << "\x1b[38;5;55m"; break;
        case RAT_U_DODGER:    std::cout << "\x1b[38;5;15m"; break;
        case RAT_AGGRESSIVE:  std::cout << "\x1b[38;5;88m"; break;
        default:              std::cout << "\x1b[38;5;153m"; break;
    }
}
...
static int read_key() {
    ...
    if (is_invulnerable) {
        ...
        switch (tolower(c)) {
            case 'w': new_dir = UP; break;
            case 's': new_dir = DOWN; break;
            case 'a': new_dir = LEFT; break;
            case 'd': new_dir = RIGHT; break;
            default: return c;
        }
...
static void get_terminal_size() {
    ...
    if (w.ws_row < Y_OFFSET + 10) {
        std::cerr << "Terminal too small - needs at least " 
                  << Y_OFFSET + 10 << " lines. Actual: " 
                  << w.ws_row << std::endl;
        exit(1);
    }    
    else {
...
static void draw_border() {
    ...
    move_cursor(0, 0);
    std::cout << symbols.border_top_left;
    for (int x = 1; x < WIDTH; x++) std::cout << symbols.border_horizontal;
    std::cout << symbols.border_top_right;
    
    for (int y = 1; y < HEIGHT; y++) {
        move_cursor(0, y);
        std::cout << symbols.border_vertical;
        move_cursor(WIDTH, y);
        std::cout << symbols.border_vertical;
    }
    
    move_cursor(0, HEIGHT);
    std::cout << symbols.border_bottom_left;
    for (int x = 1; x < WIDTH; x++) std::cout << symbols.border_horizontal;
    std::cout << symbols.border_bottom_right;
...
static const char* get_body_char(int idx) {
    ...
    else if (dy_next == -1) out_dir = UP;
    else                    out_dir = in_dir;

    if (in_dir == out_dir) {
        return (in_dir == UP || in_dir == DOWN) ? symbols.body_vertical : symbols.body_horizontal;
    }
    else if ((in_dir == UP && out_dir == RIGHT) || 
             (in_dir == LEFT && out_dir == DOWN)) {
        return symbols.body_top_right;
    }
    else if ((in_dir == UP && out_dir == LEFT) || 
             (in_dir == RIGHT && out_dir == DOWN)) {
        return symbols.body_top_left;
    }
    else if ((in_dir == DOWN && out_dir == RIGHT) || 
             (in_dir == LEFT && out_dir == UP)) {
        return symbols.body_bottom_right;
    }
    else if ((in_dir == DOWN && out_dir == LEFT) || 
             (in_dir == RIGHT && out_dir == UP)) {
...
static void draw_score_time(int score, int elapsed) {
    ...
    std::cout << "Time: " << std::setw(3) << elapsed << "s  Score: " 
              << score << "  Walls: " << wall_count << "  Bugs: " 
              << active_bugs;
    move_cursor(WIDTH - 15, HEIGHT + 1);
    if (is_invulnerable) std::cout << symbols.powerup_invulnerability << " ";
    if (speed_boost_active) std::cout << symbols.powerup_speed << " ";
    if (slow_bugs_active) std::cout << symbols.powerup_slow << " ";
        ...
        if (powerups[i].active) {
            ...
            switch (powerups[i].type) {
                case INVULNERABILITY:
                    std::cout << symbols.powerup_invulnerability;
                    break;
                case SPEED_BOOST:
                    std::cout << symbols.powerup_speed;
                    break;
                case SLOW_BUGS:
                    std::cout << symbols.powerup_slow;
                    break;
                case AREA_EXPLOSION:
                    std::cout << symbols.powerup_explosion;
                    break;
            }
        }
    }
...
static bool is_position_valid(int x, int y) {
    ...
    for (int j = 0; j < MAX_BUGS; j++) {
        if (bugs[j].active && bugs[j].x == x && bugs[j].y == y) {
            return false;
        }
    }
    
    for (int j = 0; j < MAX_POWERUPS; j++) {
        if (powerups[j].active && powerups[j].x == x && powerups[j].y == y) {
            return false;
        }
    }
...
static void generate_wall() {
    ...
    do {
        ...
        if (attempts > 100) return;
             ...
             (abs(x - snake.x[0]) + abs(y - snake.y[0]) < SAFE_ZONE_RADIUS));
...
static void spawn_bug() {
    ...
    for (int i = 0; i < MAX_BUGS; i++) {
        if (!bugs[i].active) {
            ...
            bugs[i].y = y;
            bugs[i].active = true;
            bugs[i].dir = static_cast<Direction>(rng() % 4);
            bugs[i].move_counter = 0;
            bugs[i].mode = static_cast<RatMode>(rng() % RAT_COUNT);
            bugs[i].previous_mode = bugs[i].mode;
            bugs[i].mode_timer = 20 + (rng() % 30);
            active_bugs++;
            break;
        }
    }
...
static void spawn_powerup() {
    ...
    for (int i = 0; i < MAX_POWERUPS; i++) {
        if (!powerups[i].active) {
            ...
            powerups[i].type = static_cast<PowerUpType>(dist_type(rng));
            powerups[i].spawn_time = time(nullptr);
            active_powerups++;
            break;
        }
    }
...
static void move_snake() {
    ...
    static int last_collision_x = -1;
    static int last_collision_y = -1;
    if (blocked && is_valid_direction(snake.dir)) {
        if (last_collision_x != -1) {
            ...
            if (is_wall) {
                ...
                if (last_collision_y == 0) {
                    if (last_collision_x == 0) border_char = symbols.border_top_left;
                    else if (last_collision_x == WIDTH) border_char = symbols.border_top_right;
                    else border_char = symbols.border_horizontal;
                } else if (last_collision_y == HEIGHT) {
                    if (last_collision_x == 0) border_char = symbols.border_bottom_left;
                    else if (last_collision_x == WIDTH) border_char = symbols.border_bottom_right;
                    else border_char = symbols.border_horizontal;
                } else if (last_collision_x == 0 || last_collision_x == WIDTH) {
                    border_char = symbols.border_vertical;
                }
...
static bool is_head_vulnerable() {
    ...
    int head_x = snake.x[0];
    int head_y = snake.y[0];
...
static bool coordinate_with_partner(Bug& bug) {
    if (bug.partner_index < 0 || bug.partner_index >= MAX_BUGS || !bugs[bug.partner_index].active) {
        return false;
    }
...
static void set_trap_for_snake(Bug& bug) {
    ...
    if (closest_wall >= 0) {
        int wx = walls[closest_wall].x;
        int wy = walls[closest_wall].y;
        int dx = wx - snake.x[0];
        int dy = wy - snake.y[0];
        
        if (abs(dx) > abs(dy)) {
            bug.dir = (dx > 0) ? LEFT : RIGHT;
        } else {
            bug.dir = (dy > 0) ? UP : DOWN;
        }
    } else {
        int dx = snake.x[0] - bug.x;
        int dy = snake.y[0] - bug.y;
...
static void move_bugs() {
    for (int i = 0; i < MAX_BUGS; i++) {
        ...
        if (bugs[i].mode_timer-- <= 0) {
            ...
            int total_weight = 0;
            for (int w : mode_weights) total_weight += w;
            
            int r = rng() % total_weight;
            int new_mode = 0;
            while (r >= mode_weights[new_mode]) {
                r -= mode_weights[new_mode];
                new_mode++;
            }
            bugs[i].mode = static_cast<RatMode>(new_mode);
            bugs[i].mode_timer = 50 + (rng() % 100);
        ...
        switch (bugs[i].mode) {
            case RAT_ERRATIC:
                ...
                if (rng() % 3 == 0) {
                    ...
                    if (abs(dx) > abs(dy)) {
                        bugs[i].dir = (dx > 0) ? RIGHT : LEFT;
                    } else if (dy != 0) {
                        bugs[i].dir = (dy > 0) ? DOWN : UP;
                    }
                }
                break;
            ...
            case RAT_STALKER: {
                ...
                if (is_head_vulnerable() && (abs(dx) + abs(dy) <= 10)) {
                    ...
                    for (int j = 1; j < snake.length; j++) {
                        if (abs(snake.x[j] - bugs[i].x) <= 2 && 
                            abs(snake.y[j] - bugs[i].y) <= 2) {
                            avoid_segment = true;
                    ...
                    if (!avoid_segment) {
                        if (abs(dx) + abs(dy) < 5) {
                            if (rng() % 2 == 0) {
                                bugs[i].dir = (dx > 0) ? LEFT : RIGHT;
                            } else {
                                bugs[i].dir = (dy > 0) ? UP : DOWN;
                            }
                        } else {
                            if (abs(dx) > abs(dy)) {
                                bugs[i].dir = (dx > 0) ? RIGHT : LEFT;
            ...
            case RAT_AMBUSHER:
                if (bugs[i].target_fruit >= 0 && bugs[i].target_fruit < fruit_count && 
                    fruits[bugs[i].target_fruit].active) {
            ...
            case RAT_PACK_HUNTER:
                if (bugs[i].partner_index >= 0 && bugs[i].partner_index < MAX_BUGS && 
                    bugs[bugs[i].partner_index].active) {
                    if (coordinate_with_partner(bugs[i])) {
                        force_move = true;
                    }
                } else {
                    bugs[i].mode_timer = 0;
            ...
            case RAT_COUNT:
                bugs[i].mode = RAT_ERRATIC;
                break;

            case RAT_AGGRESSIVE: {
                bool can_attack_head = false;
                for (int j = 0; j < 3 && j < snake.length; j++) {
                    int dist = abs(snake.x[j] - bugs[i].x) + abs(snake.y[j] - bugs[i].y);
                    if (dist <= 5) { 
                        dx = snake.x[j] - bugs[i].x;
                        dy = snake.y[j] - bugs[i].y;
                ...
                if (!can_attack_head) {
                    ...
                    for (int j = 1; j < snake.length; j++) {
                        if (abs(snake.x[j] - bugs[i].x) <= 2 && 
                            abs(snake.y[j] - bugs[i].y) <= 2) {
                            dx = bugs[i].x - snake.x[j];
                            dy = bugs[i].y - snake.y[j];
        ...
        if (!force_move && slow_bugs_active) {
            if (rng() % 2 == 0) continue;
        }
...
static void check_eat_fruits() {
    for (int i = 0; i < fruit_count; i++) {
        if (fruits[i].active && snake.x[0] == fruits[i].x && snake.y[0] == fruits[i].y) {
            ...
            if (fruits_eaten % 5 == 0 && fruits_eaten > 0) {
                generate_wall();
            }
            ...
            if (fruits_eaten % 10 == 0) {
                ...
                for (int j = 0; j < bugs_killed_by_body; j++) {
                    spawn_bug();
                }
        ...
        if (powerups[i].active && snake.x[0] == powerups[i].x && snake.y[0] == powerups[i].y) {
            ...
            switch (powerups[i].type) {
                ...
                case SPEED_BOOST:
                    ...
                    break;
                case SLOW_BUGS:
                    slow_bugs_active = true;
                    slow_bugs_timer = 120;
                    break;
                case AREA_EXPLOSION:
                    for (int j = 0; j < MAX_BUGS; j++) {
                        if (bugs[j].active && 
                            abs(bugs[j].x - snake.x[0]) <= 3 && 
                            abs(bugs[j].y - snake.y[0]) <= 3) {
...
static void update_powerups() {
    ...
    for (int i = 0; i < MAX_POWERUPS; i++) {
        if (powerups[i].active) {
            if (current_time - powerups[i].spawn_time > 20) {
    ...
    if (is_invulnerable && --invulnerability_timer <= 0) {
        is_invulnerable = false;
    }
    if (speed_boost_active && --speed_boost_timer <= 0) {
        speed_boost_active = false;
    }
    if (slow_bugs_active && --slow_bugs_timer <= 0) {
        slow_bugs_active = false;
    }
}
...
static void init_stats() {
    ...
    for (int i = 0; i < RAT_COUNT; i++) {
        game_stats.bugs_by_type[i] = 0;
    }
}

static void update_stats() {
    game_stats.total_time = static_cast<int>(time(nullptr) - start_time);
    game_stats.max_length = std::max(game_stats.max_length, snake.length);
...
static void print_with_delay(const std::string& text, int delay_ms = 30) {
    for (char c : text) {
        std::cout << c << std::flush;
...
static void show_stats() {
    ...
    std::string title = "FINAL STATISTICS";
    int title_padding = (BOX_WIDTH - 2 - title.length()) / 2;
    std::cout << "║" << std::string(title_padding, ' ') << title 
              << std::string(BOX_WIDTH - 2 - title_padding - title.length(), ' ') <<  "║\n"
              << "║" << std::string(BOX_WIDTH - 2, ' ') << "║\n";
    ...
    auto write_stat_line = [&](int line_num, const std::string& label, const std::string& value, bool is_substat = false) {
        std::cout << "\x1b[" << (5 + line_num) << ";3H" << std::flush;
        std::string display_label = is_substat ? "  " + label : label;
        for (size_t i = 0; i < display_label.length(); i++) {
            std::cout << display_label[i] << std::flush;
            std::this_thread::sleep_for(std::chrono::milliseconds(30));
        }
        int current_pos = 3 + display_label.length();
        int dots_needed = VALUE_COL - current_pos;
        if (dots_needed > 0) {
            std::cout << std::string(dots_needed, '.') << std::flush;
        }
        std::cout << "\x1b[" << (5 + line_num) << ";" << VALUE_COL << "H" << value;
        
        std::cout << " " << std::flush;
        std::this_thread::sleep_for(std::chrono::milliseconds(LINE_DELAY));
    };
    int current_line = 0;
    write_stat_line(current_line++, "Total time", format_time(game_stats.total_time));
    write_stat_line(current_line++, "Fruits eaten", std::to_string(game_stats.fruits_eaten));
    write_stat_line(current_line++, "Max length", std::to_string(game_stats.max_length));
    write_stat_line(current_line++, "Bugs eliminated", std::to_string(game_stats.bugs_killed));
    for (int i = 0; i < RAT_COUNT; i++) {
        if (game_stats.bugs_by_type[i] > 0) {
            ...
            switch (i) {
                ...
                default:              label = "Type " + std::to_string(i); break;
            }
            write_stat_line(current_line++, label, std::to_string(game_stats.bugs_by_type[i]), true);
        }
    }
    
    write_stat_line(current_line++, "Direction changes", std::to_string(game_stats.direction_changes));
    write_stat_line(current_line++, "Power-ups collected", std::to_string(game_stats.powerups_collected));
    write_stat_line(current_line++, "Bugs remaining", std::to_string(active_bugs));
    
    std::cout << "\x1b[" << (7 + total_content_lines) << ";0H" << std::flush;
}
...
static void draw_debug_info() {
    ...
    move_cursor(0, HEIGHT + 2);
    std::cout << "DEBUG: Head(" << snake.x[0] << "," << snake.y[0] << ") ";
    std::cout << "Dir: ";
    switch (snake.dir) {
    ...
    std::cout << " Invul: " << is_invulnerable << " ";
    
    move_cursor(0, HEIGHT + 3);
    std::cout << "Bugs: ";
    for (int i = 0; i < MAX_BUGS; i++) {
        if (bugs[i].active) {
            std::cout << i << ":(" << bugs[i].x << "," << bugs[i].y << ",";
            switch (bugs[i].mode) {
                case RAT_ERRATIC: std::cout << "ERR"; break;
                case RAT_STALKER: std::cout << "STK"; break;
                case RAT_AMBUSHER: std::cout << "AMB"; break;
                case RAT_PANIC: std::cout << "PAN"; break;
                case RAT_PACK_HUNTER: std::cout << "PKH"; break;
                case RAT_TRAP_MASTER: std::cout << "TRP"; break;
                case RAT_U_DODGER: std::cout << "UDG"; break;
                case RAT_AGGRESSIVE: std::cout << "AGR"; break;
...
int main() {
    ...
    while (true) {
        std::this_thread::sleep_for(std::chrono::microseconds(
            speed_boost_active ? LOGIC_DELAY_US / 2 : LOGIC_DELAY_US));
        tick_count++;
        ...
        if (kbhit()) {
            ...
            if ((c == 'w' || c == 'W') && snake.dir != DOWN) snake.dir = UP;
                ...
                (void)getch();
                start_time += time(nullptr) - pause_start;
            }
        }
